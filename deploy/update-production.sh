...
run_update() {
    ...
    # Check for malformed .env before sourcing
    if grep -q "=[^\"].* " "$DEST_DIR/go-server/.env"; then
        echo -e "${RED}❌ Malformed .env detected: Values with spaces MUST be wrapped in quotes.${NC}"
        echo -e "${YELLOW}Please check SMTP_PASS or other variables with spaces.${NC}"
        exit 1
    fi
