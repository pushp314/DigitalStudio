...
const OAuthButton = ({ provider, icon, onClick, variant = 'default' }) => {
    
    // Default Icons if none provided
    const getIcon = () => {
        if (icon) return icon;
        if (provider.toLowerCase() === 'github') return <Github size={20} />;
    ...
    return (
        <button
            type="button"
            onClick={onClick}
            className={`flex items-center justify-center gap-4 w-full font-bold py-3.5 rounded-xl transition-all shadow-sm active:scale-[0.98] ${
                isSlate 
