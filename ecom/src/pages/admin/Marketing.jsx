...
const Marketing = () => {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 pb-20" style={{ fontFamily: "'Inter', sans-serif" }}>
            ...
            <div className="flex flex-col lg:flex-row justify-between items-center gap-6 bg-white p-6 sm:p-8 rounded-[2rem] sm:rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden">
                ...
                <div className="relative z-10 text-center lg:text-left">
                    <h2 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight mb-1 flex items-center justify-center lg:justify-start gap-3">
                        Marketing Console <Activity size={20} className="text-emerald-500" />
                    </h2>
                    <p className="text-[10px] sm:text-xs text-slate-500 font-medium">Orchestrate marketplace promotions and manage discount codes.</p>
                </div>
                {!isCreating && (
                    <button 
                        onClick={() => setIsCreating(true)}
                        className="w-full lg:w-auto px-8 py-3.5 bg-slate-900 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all shadow-xl shadow-slate-200 flex items-center justify-center gap-3 relative z-10 hover:scale-105 active:scale-95"
                    >
            ...
            {/* Creation Surface */}
            {isCreating && (
                <div className="bg-slate-900 text-white p-6 sm:p-10 rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl border border-slate-800 animate-in zoom-in-95 duration-300 relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <div className="flex justify-between items-center mb-8 sm:mb-10">
                            <div>
                                <h3 className="text-base sm:text-lg font-black uppercase tracking-widest mb-1">Coupon Configuration</h3>
                                <p className="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-widest">Administrator Marketing Access</p>
                            </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                            <button 
                                ...
                                className="px-10 py-4 bg-white text-slate-900 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-emerald-400 transition-all shadow-xl shadow-emerald-500/10 flex-1"
                            >
                                Create Coupon
                            </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {coupons?.map(coupon => (
                    <div key={coupon.id} className={`group relative bg-white p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border-2 transition-all duration-500 shadow-sm hover:shadow-2xl overflow-hidden ${coupon.active ? 'border-slate-50 hover:border-slate-900' : 'opacity-60 grayscale border-slate-100 bg-slate-50/50'}`}>
                        {editingId === coupon.id ? (
                            ...
                            <>
                                <div className="flex justify-between items-start mb-6 sm:mb-8">
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-3 mb-2">
                                            <div className={`w-10 h-10 rounded-2xl shrink-0 flex items-center justify-center font-black text-sm shadow-sm transition-colors ${coupon.active ? 'bg-slate-900 text-white' : 'bg-slate-200 text-slate-400'}`}>%</div>
                                            <div className="min-w-0">
                                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-0.5">Modifier</p>
                                                <h3 className="text-sm sm:text-base font-black text-slate-900 truncate tracking-tight">{coupon.code}</h3>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-1.5 sm:translate-x-2 sm:-translate-y-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-all duration-300">
                                        ...
                                        <ActionButton 
                                            ...
                                            title={coupon.active ? "Pause" : "Activate"} 
                                        />
                                        <ActionButton icon={<Trash2 size={14} />} onClick={() => setConfirmModal({ isOpen: true, id: coupon.id })} color="text-rose-500" title="Delete" />
                                    </div>
                                </div>

                                <div className="mb-6 sm:mb-8">
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Total Impact</p>
                                    <h4 className="text-3xl sm:text-4xl font-black text-slate-900 tracking-tighter">
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-6">
                                    <StatItem label="Min. Spend" icon={<Zap size={10} />} value={`₹${coupon.minPurchase}`} />
                                    <StatItem label="Total Usage" icon={<ArrowUpRight size={10} />} value={`${coupon.usageCount || 0}/${coupon.usageLimit || '∞'}`} />
                                    <StatItem label="Expiry" icon={<Clock size={10} />} value={coupon.expiresAt ? new Date(coupon.expiresAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' }) : 'Permanent'} />
                                ...
                                {!coupon.active && (
                                    <div className="absolute inset-0 bg-white/40 backdrop-blur-[1px] rounded-[2rem] sm:rounded-[2.5rem] flex items-center justify-center p-6 text-center select-none pointer-events-none">
                                        <p className="text-[10px] sm:text-[11px] font-black text-slate-900 border-2 border-slate-900 px-6 py-2.5 rounded-full rotate-[-8deg] shadow-2xl bg-white/80 uppercase tracking-[0.4em]">Node Revoked</p>
                                    </div>
                                )}
                            </>
                        )}
                ...
                {(!coupons || coupons.length === 0) && !isLoading && (
                    <div className="col-span-full py-20 sm:py-32 border-4 border-dashed border-slate-50 rounded-[2rem] sm:rounded-[3rem] text-center">
